...
class WordTrigger(Trigger):
    ...
    def isWordIn(self, story):
        listStory = self.__storySplit(story)
        if self.word in listStory:
            return True
        return False
...
class PhraseTrigger(Trigger):
    ...
    def evaluate(self, newsStory):
        storyArray = [newsStory.getTitle(), newsStory.getSubject(), newsStory.getSummary()]
        for text in storyArray:
            if str(self.phrase) in str(text):
                return True
        return False
...
def filterStories(stories, triggerlist):
    ...
    for story in stories:
        for trigger in triggerlist:
            if trigger.evaluate(story) and not story in filtered:
...
def readTriggerConfig(filename):
    ...
    for line in lines:
        ...
        if linesplit[0] != "ADD":
            trigger, triggerMap = makeTrigger(triggerMap, linesplit[1], linesplit[2:], linesplit[0])
